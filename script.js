...
async function loadDynamicContent() {
  ...
  try {
    ...
    // Reconstruct data into array of simple objects mapping to column headers
    const cols = json.table.cols.map(c => c && c.label ? c.label.toLowerCase().trim() : '');
    const data = json.table.rows.map(row => {
      const obj = {};
      cols.forEach((col, i) => {
        if (col) {
          obj[col] = row.c[i] && row.c[i].v !== null ? row.c[i].v : '';
        }
      });
      return obj;
    });
